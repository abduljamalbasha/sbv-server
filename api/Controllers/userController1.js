...
const uploadfunction = require("../Feature/UploadFiles");
...
router.get("/userDetail/:userid", (req, res) => {
  connection.query(
    "select * from `UsersDetail` where Id = ? LIMIT 3",
    req.params.userid,
    (err, result) => {
      if (err) throw err;
      res.json(result[0]);
...
router.post("/userDetail", uploads.single("userImage"), (req, res, next) => {
  ...
  //Uploading your image to GCP Storage
  const customizefilename = `${Date.now()}-${req.file.originalname}`;
  uploadfunction.UploadYouFiles(req, customizefilename);
  ...
  const UsersDetail = {
    ...
    userImage: customizefilename //req.file.originalname
  };
